"""Tool implementations for ``nook-mcp`` — the *uninstrumented* "before".

This module is a line-for-line counterpart to :mod:`nook_mcp.tools_with_otel`
with every trace of OpenTelemetry removed: no ``opentelemetry`` imports, no
tracer, no spans, and no span attributes. It exists so we can show a clean
"before" (this file) and "after" (``tools_with_otel.py``) side by side. It is
used as the workshop starting point, to progressively add instrumentation.

Registration happens via :func:`register_tools`, which receives the
``FastMCP`` instance from :mod:`nook_mcp.server` and attaches each tool with
the ``@mcp.tool`` annotation.

Note: even without OTel in this module, traces may still appear in Jaeger
because ``kafka_client.py`` opens its own spans and FastMCP auto-instruments
the ``tools/call`` boundary. This file only removes *application-level*
instrumentation from the tool bodies.
"""
...
async def _simulate_llm_reasoning(*, room: str, bells: int, multipliers: dict[str, float]) -> str:
    ...
    Sleeps to mimic latency so the trace shape stays realistic even offline.
    """
...
async def _llm_reasoning(*, room: str, bells: int, multipliers: dict[str, float]) -> str:
    """Call the local Ollama (Qwen) model to narrate Tom Nook's quote."""
    payload = {
        "model": _OLLAMA_MODEL,
...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def shake_tree(tree: str) -> dict:
        ...
        raise ValueError(
            f"Unknown tree type: {tree!r}. Valid types: cedar, hardwood, apple, cherry, orange, peach, pear, coconut palm."
        )
    ...
    async def query_topic(
        ...
        records = await kafka_client.query_topic_messages(topic, key_filter=key_filter, since_iso=since, limit=limit)
        return {"topic": topic, "count": len(records), "records": records}
    ...
    async def publish_event(
        ...
        """
        return await kafka_client.publish_event_to_topic(topic, payload, key=key)
